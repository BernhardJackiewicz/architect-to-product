...
**25 MCP tools** · **959 tests** · **Architecture → Plan → Build (evidence-gated) → Quality Audit (cadence) → Code Review → Signoff → E2E Testing → Security → Whitebox → Verify → Release Audit → Deploy → Backup**
...
[![Tests: 951 passing](https://img.shields.io/badge/tests-959%20passing-brightgreen)]()
...
5. **Whitebox Audit**: Analyzes whether SAST findings are actually exploitable — checks reachable code paths, missing guards, trust boundaries, prompt-only enforcement. Blocking findings prevent deployment (enforced in code, not just prompts). **19 deterministic probes** detect hardcoded secrets, SQL/command/NoSQL injection, XSS, insecure deserialization, eval with user input, SSRF, path traversal, mass assignment, open redirects, insecure crypto, cookie security, missing auth/rate limiting/input validation — each with context-aware suppression to reduce false positives. **Evidence-based adversarial review** across 15 domains (business logic, auth bypasses, race conditions, IDOR/ownership, XSS, deserialization, cookie/CORS security, deployment config, backup security): high/critical findings require confidence level and file:line evidence. Hypotheses are auto-downgraded to medium. Confidence stats track evidence quality across rounds.
