...
export function handleGetChecklist(input: GetChecklistInput): string {
  ...
  const checklist = {
    preDeployment: [
      ...
      { item: "Secret management tier chosen (.env / Docker Swarm / Infisical / Vault)", done: false },
      { item: ".env.production permissions set to 600 (owner-only readable)", done: false },
      { item: "Production secrets not stored inside Docker image", done: false },
    ...
    infrastructure: [
      ...
      { item: "SSL/HTTPS verified via a2p_verify_ssl (required gate)", done: !!state.sslVerifiedAt },
    ],
    postDeployment: [
      { item: "/health returns OK", done: false },
      { item: "/.env returns 403/404 (blocked by Caddy)", done: false },
      { item: "/.git/ returns 403/404", done: false },
      { item: "HTTPS enforced (HTTP redirects to HTTPS)", done: !!state.sslVerification?.httpsRedirect },
      { item: "HSTS header present", done: !!state.sslVerification?.hstsPresent },
      { item: "Security headers present (X-Frame-Options, X-Content-Type-Options, etc.)", done: false },
